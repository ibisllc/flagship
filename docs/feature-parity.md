...
| 2 | Username taken-state + **trademark-claim** | ✅ | ⚠️ | ❌ | iOS+Android | **P2** |
...
| 6 | Key backup + Secure-your-account + **reminder** | ⚠️ | ⚠️ | ⚠️ | all 3 (reminder) | **P1** |
| 7 | Create server (QR relay, TTL, boot-unlock) | ✅ | ✅ | ✅ | — | — |
| 8 | Pending server + cancel → **release name** | ✅ | ⚠️ | ⚠️ | iOS+Android | **P3** |
...
| 20 | **Tier-status / monetization** | ✅ | ❌ | ❌ | iOS+Android | **P7** |
...
- **To iOS + Android:** trademark-claim (P2), audit log (P5), collaborator
  invites (P6), tier-status (P7), browser-viewer (P8), peer-backup (P9),
  server-release (P3, verify), cross-device pairing → Android only (P4).
- **To webapp:** Replace device (P10), Wipe & restart (P11), multi-profile (P12).
- **All three:** post-creation backup reminder (P1), kill-switch UI (P13).
- **Verify-only:** marketplace iOS wiring, create-server iOS pickers, trademark
  iOS state (P0).
