...
5. **§3.4 single per-user leftmost-label resolver** — daemon — M — *extends* (`runtime.ts:510-536`/`userZoneOf`/`leftmostLabel`): 4-step precedence `--`pin → box-name → install-table app-label → disambiguation page (devices are not publicly addressable). **Blocked on Q3.**
6. **Merged per-user name-uniqueness invariant** — storage/.com — M — *net-new* (`storage/schema.ts:12-42`: uniqueness across {app-label, box-name} per user + stable-id↔local-label map). "The one genuinely new cross-cutting invariant." **Blocked on Q3.**
...
- **Q3 — Merged per-user leftmost-label namespace (§3.4, §9).** {app-label, box-name} are 2 separate sources today; need merged table or deterministic precedence + uniqueness across both. Device labels are NO LONGER a source — devices are not publicly addressable (see the banner on `v2-device-addressing-and-real-ticket`). Central blocker for tasks 5 + 6.
