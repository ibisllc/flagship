# Session handoff — 2026-06-02 (icons · teal · recovery · builder certAutonomy)
...
- **Builder `certAutonomy` bug fixed** — see ⏸️/🧪 below. Commits **`6aac36c`** (code+tests) + **`db36cc6`** (doc).
- **Mac Flagship Studio rebuilt + reinstalled** with the builder fix (Developer-ID signed, in `/Applications`, running).

## ⏳ Owner must confirm on a real device (cannot close from a dev box)
1. **Cross-device QR recovery fix** — commit **`f4593a3`** added `.preferImmediatelyAvailableCredentials` to the recovery passkey assertion (`apps/mobile/ios/Sources/FlagshipUI/Components/PlatformWebAuthnProvider.swift`, both `prfAssert` + `assertAny`). **Never confirmed on device** (owner pivoted to wipe/icons/builder).
...
3. **A real burn → box registers → green padlock** — builder is fixed + running; the `office.harry` recipe is valid until **2026-06-03 03:46 CDT**.
...
- **Windows builder (C#) — NOT compiled here (no `dotnet` on the build Mac).** `apps/builder-windows/src/Recipe.cs` + `AlpinePersonalize.cs` + `tests/RecipeTests.cs` got the `certAutonomy` fix mirroring the **verified** Swift change. Run `dotnet test apps/builder-windows` on a machine with the SDK. *(Mac side IS verified: `swift test` 76/76, incl. new `certAutonomy` golden vectors.)*
...
- **Builder field trap (root cause of the certAutonomy bug):** any new optional install-blob field must be added in **BOTH** the canonical-bytes builder **AND** the trailer serializer, in **every** builder (TS `flagship-builder` + iso-personalizer `trailer.ts`, Mac `apps/builder-mac`, Windows `apps/builder-windows`) — or the local burn "succeeds" but the Worker's re-verify at `/api/server/register` rejects the box. The implementation table in `docs/recipe-schema-v2.md` was updated this session to list all of them.
- **Mac builder rebuild:** `cd apps/builder-mac && FLAGSHIP_SIGNING_ID="Developer ID Application: IBIS LLC (8G8RHBU9BN)" make sign`, then `ditto .build/release/"Flagship Studio.app" /Applications/`. Signed-not-notarized is fine locally (no quarantine flag on a `ditto`-copied local build). A rebuilt **helper** binary may require re-approving Login Items + Full Disk Access on the first burn.
