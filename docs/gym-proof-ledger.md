...
- **#89 iOS vibe-code CHAT — DONE + PROVEN LIVE (box home.wapkcr6i5):** real app → BYOK AI-key step → live gpt-4o-mini streamed + ASKED a question → the test typed a real chat reply guiding it → AI authored the service → Deploy → `curl` **HTTP 200 `hello from ios chat`** (`TEST SUCCEEDED 1/1`). The chat-build-deploy path was NON-functional before — fixed 3 gaps incl. **2 prod daemon bugs**: a `/reply` never re-invoked the LLM (chat stalled after the first question), and the stream parser was left `done` so resumed files were dropped → deploy 502. Daemon multi-turn fix on main `3bbfb898`; iOS client (chat Deploy + live screens-client + hand-off) `ffe9b7f6`. Gym XCUITest `GymLiveVibeChatTests` stays on `origin/finale-ios-chat`. Artifacts: `gym-results/ios-chat-proof/` (10 screenshots + deployed-200.txt). NOTE: the box clones `main`, which now has the daemon fix → real boxes get the chat-build path on next provision.
- **#90 Android vibe-code CHAT — DONE + PROVEN LIVE (box wapkekp0c):** mirror of iOS — real app on the `flagship_gym` emulator → BYOK AI-key → live gpt-4o-mini streamed + asked → real in-app chat reply → Deploy → `curl` **HTTP 200 `hello from android chat`** (`OK (1 test)`). Client on main `d6ac57c6` (vibeCodeDeploy + chat hand-off). +3 real Android prod bugs fixed: a `LiveScreensClient` background-thread resume → main-thread crash (→ `Dispatchers.IO`), a `NetworkOnMainThreadException`, and a dead off-screen "Build it" (→ scrollable). Gym adoption seam + instrumentation test stay on `origin/finale-ios-chat`'s Android sibling.
- **#92 service access gating (open/restricted + capability invite links)** — spec `docs/service-access-gating.md`. Identity = **AID** (`deriveAccountId(UMK)`, `flagship/account-id/v1`, NON-rotating; the IRK is versioned/rotates so it's NOT the identity — owner-caught). **BACKEND COMPLETE + WEBAPP CLIENT ON MAIN** (`5ddb8aec`..`9cf7e5e5` backend, `230b9cbd` webapp, `b601c17a` GET+cookie; tsc clean, full vitest 5879 pass): protocol (AID, household-key bundle, create/redeem/revoke envelopes), `.com` `service_invites` store (migration 0056) + handlers (first-bind/same-AID-idempotent/reject-different/revoke), daemon `access.mode` (default open) + **AID-signed `x-flagship-visit` OR `Flagship-App-Session` cookie** enforcement (cookie bound to AID+service, re-checks the allow-list per request so a `.com` revoke kills it; issued on redeem + `POST /api/service-access/establish-session`) + `GET /api/service-access/<ref>` (unauth, exposes only `allowCount`, never the AID graph). Webapp: crypto mirror (byte-parity-verified vs `@flagship/protocol`) + cross-platform vector fixture (`packages/protocol/tests/fixtures/serviceAccessGating.vectors.json`) + admin toggle/allow-list/invite UI + friend `/invite#secret` deep-link. flagshipserver.com is bundle-blind (no UMK). **ALL 3 CLIENTS NOW ON MAIN** (`f6a371a9`..`304f277f`): iOS + Android crypto mirrors (AID/household/AEAD, fixture-pinned byte-parity) + `ServiceAccessClient` + admin allow-list + friend-redeem UI + `flagship://invite` deep-link (iOS 1089 / Android 937 pass). **Remaining:** the gym test (admin restrict+invite → friend redeems → access → revoke → denied; + an **IRK-rotation-keeps-access** assertion that proves the AID choice) — BLOCKED on syncing the deployed gym env to current main.
- **Web-experience gating (QR-login for restricted-service WEBSITES) — owner-designed 2026-06-19; DAEMON BACKEND ON MAIN, clients in flight.** A browser can't AID-sign, so a restricted service's website was unreachable from one. WhatsApp-Web-style QR-login: box serves a self-contained **knock page** (server-side QR via `qrSvg.ts`, byte-for-byte port of the webapp encoder) on a deny → phone **AID-signs** `KnockAuthorization{serverId,serviceRef,pageId,…}` (`flagship/service-knock/v1`; pageId in the sig — no replay onto another page) → box mints a session cookie for the browser + a phone-held `secretId`. **Holder cookie** closes the pageId-theft race (only the requesting browser gets the session cookie); session **status/close** is secretId-in-BODY, rate-limited ~1/min, default-offline (no oracle). Backend `3343aeb9` (`serviceAccessWeb.ts` + `qrSvg.ts` + protocol envelope + 19 new tests; server-daemon+protocol 2120 pass, tsc clean). Spec + the 6 owner-design catches folded in: `docs/service-access-gating.md` § "Web-experience gating". **ALL 3 CLIENTS NOW ON MAIN** (3 parallel worktree workers off `f42ad08e`, cherry-picked clean — disjoint surfaces): **iOS** `b1c13e2f..5c69595b` (`flagship://access` deeplink + KnockAuthorize screen behind Face-ID + `SecuredSessionStore` + Settings "Open secured sessions"/"Process URL"; xcodebuild 1118 XCTests + App build green), **Android** `45f9c054..30fab27e` (mirror; `:app:testDebugUnitTest` 961 + assembleDebug green), **webapp** `75f010ca..4cdb335d` (paste-to-authorize view + secured-sessions page; vitest +41). Each pins `signKnockAuthorization` byte-for-byte against the shared `"knock"` fixture vector. Whole-tree on main after the wave: **tsc -b clean · full vitest 446 files / 5916 pass**. **Remaining = the live gym validation** (drive the QR-login + invite/redeem flow against a real box: admin restrict → invite → friend redeem → browser knock → phone authorize → access → revoke → denied + IRK-rotation-keeps-access) — bills + lengthy, owner-timing-gated.
