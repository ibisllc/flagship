...
- **Expanded AI-build scope (user, 2026-06-18) — DO ALL:**
  - **Multi-tier URLs (#88) — RESULT (box home.wapk13lvf, 2026-06-18):** canonical `<svc>.home.<user>` **PROVEN 200** for BOTH the vibe + git app (`curl -i` + cert chain = per-box wildcard `[*.home.<user>, home.<user>]`, LE YR1). The other two tiers are **NOT WIRED in the gym env — they're real BUILDS, not tests:**
    - **short-canonical `<svc>.<user>` (tier-2) BLOCKED — 3 evidenced gaps:** (1) **CODE gap**: `serviceCertHttp.ts` calls `buildServiceCertHandlers` with NO `apex` → hardcodes `"flagship.services"` ≠ gym `gym.flagship.services` → daemon 403 "not a tier-2 name". (2) gym DNS broker has no tier-2 authority path → 403 "service-cert authority not supported here". (3) per-box DNS publishes only `[<server>.<user>, *.<server>.<user>]` → `<svc>.<user>` has NO DNS record + the hub tunnel-claim covers only the per-box names → no leader route. (matches the standing "tier-2 mint/install" + "user-zone A record dropped under per-box DNS" follow-ups.)
    - **voi.ci prod-only:** gym CREATE API works (`POST gym.flagshipserver.com/api/voici/shorten` → 200, returns `voi.ci/<code>`) but `voi.ci/*` is routed to the PROD Worker reading PROD's D1 (a gym code never resolves), and `voi.ci`'s origin returns **523** (no working origin even for prod). So canonical is the ONLY tier wired in gym.
    - Artifacts: `gym-results/multitier-url-proof/` (raw curl per tier/app, mint verdicts, cert chain, voici probe). Tooling on `finale-multitier` (`7bae8e3a`): `vibe-drive.ts` + `multitier-drive.ts`.
