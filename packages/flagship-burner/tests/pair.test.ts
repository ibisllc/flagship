/**
 * `flagship-burn pair` transport glue.
 *
 * Drives `runPair` through a fake duplex transport (no real socket): the full
 * relay handshake (burner-hello → phone-hello → SAS → confirm → deliver), then
 * asserts the recipe is written + signature-verified. The `--debug` path is
 * exercised end-to-end: the burner requests consent, the "phone" returns an
 ...
 *
 * The phone/relay are simulated: the burner sends BARE app frames; the relay
...
describe("runPair transport glue", () => {
  it("completes the handshake and writes a verified recipe (no debug)", async () => {
    ...
    harness.open();
    harness.inject({ kind: "accepted", role: "burner" });
    harness.inject({ kind: "peer-joined" });
    await tick();
    // The burner hands the phone its pubkey so a typed-code phone can derive the SAS.
    const hello = harness.lastOf("burner-hello");
    expect(hello?.burnerPk).toBe(base64UrlEncode(burnerKp.publicKey));
  ...
  it("ignores a deliver that arrives before pairing (no aead key yet)", async () => {
    ...
    const p = runPair({ out, quiet: true, keypair: burnerKp, transport: () => harness.transport });
    harness.open();
    harness.inject({ kind: "peer-joined" });
    await tick();
  ...
  it("--debug requests consent and embeds the owner-signed debugGrant", async () => {
    ...
    // Burner must now ask the phone to approve debug access — NOT finalize yet.
  ...
  it("--debug with a declined consent burns a production image (no grant)", async () => {
    ...
    const p = runPair({ out, quiet: true, debug: true, consentTimeoutMs: 5_000, keypair: burnerKp, transport: () => harness.transport });
    harness.open();
    harness.inject({ kind: "peer-joined" });
    await tick();
    harness.inject({ kind: "peer", frame: { kind: "phone-hello", phonePk: base64UrlEncode(phoneKp.publicKey) } });
  ...
  it("rejects when the relay session expires before any recipe", async () => {
    const burnerKp = newBurnerKeypair();
    const harness = fakeTransport();
    const p = runPair({ out: join(tmpdir(), "never.json"), quiet: true, keypair: burnerKp, transport: () => harness.transport });
    harness.open();
    harness.inject({ kind: "expired" });
    await expect(p).rejects.toThrow(/timed out/);
  });
