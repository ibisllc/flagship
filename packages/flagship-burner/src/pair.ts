 ...
 *
 * ADVANCED — debug access (`--debug`): mirrors the macOS burner's Advanced
 * "Debug mode" toggle. Enabling a box's debug console user / SSH is NOT a
 * burner checkbox — it requires an owner-IRK-signed grant the BOX verifies.
 * With `--debug`, after the recipe arrives the burner sends a `consent-request`
 * over the session; the phone shows a security warning + Face ID and replies
 * with a signed `flagship/debug-access/v1` grant; the burner embeds it as the
...
export interface PairOptions {
  ...
  codeBytes?: Bytes;
  /** Fixed burner keypair (default: random). */
