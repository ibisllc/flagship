...
// A REAL signed v2 recipe so the harness exercises verify_recipe_signature too
// (the live installer now fail-closes on an unsigned/tampered recipe). We sign
// with a throwaway IRK; the registration that would consume the serial runs on
// the installed OS (out of scope here), so a synthetic serial is fine.
const irk = deriveIRK(generateUMK());
const now = Date.now();
const authCode = {
  version: 1,
  serial: "E2EHARNESS0001",
  username: "e2e",
  serverName: "e2e",
...
const blob = {
  ...
  registrationUrl: "http://127.0.0.1:1",
  authCode,
...
const recipe = JSON.stringify(installBlobToJson(blob));
...
const env = [
  ...
  "export TARGET=/dev/vda", // the blank 12G target disk QEMU attaches (only virtio disk)
...
const launcher = [
  ...
  'echo "target /dev/vda present: $( [ -b /dev/vda ] && echo yes || echo NO )"',
  ...
  "sgdisk -p /dev/vda 2>/dev/null || true",
  "poweroff -f",
  "",
].join("\n");
...
const tar = buildApkovl({
  ...
  files: [
    ...
    { name: "flagship/install-blob.sig", content: new Uint8Array(blobSig), mode: 0o600 },
