/**
 * Phone↔builder pairing crypto — the builder (initiator) side, in TS.
 *
 * This is the cross-platform mirror of apps/builder-mac BuilderPairing.swift
 * (Swift) + apps/mobile/android core/BuilderPairing.kt (Kotlin). It lets a
 * Node builder (packages/flagship-builder `pair`) run the SAME pairing the
 * macOS Swift builder does, so a Linux / Chromebook box can pair with the
 ...
 * mapping uses `flagship/builder-sid/v1`. All pinned by builderPairing.test.ts
 * (and apps/com/test/builderPairingVector.test.ts).
 */
...
const SID_TAG = enc("flagship/builder-sid/v1");
...
export function qrPayload(human: string, builderPub: Bytes): string {
  return `flagship://builder?c=${human}&k=${base64UrlEncode(builderPub)}`;
}
...
export function newBuilderKeypair(): BuilderKeypair {
...
export function deriveSessionMaterial(builderSecret: Bytes, phonePublicKey: Bytes): SessionMaterial {
  const shared = x25519.getSharedSecret(builderSecret, phonePublicKey);
