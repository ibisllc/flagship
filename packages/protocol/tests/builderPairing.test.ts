...
// Pinned cross-platform vector — must match apps/builder-mac BuilderPairingTests,
// the iOS/Android DebugAccess+BuilderPairing tests, and apps/com builderPairingVector.
const BUILDER_PRIV = hex("01".repeat(32));
const PHONE_PRIV = hex("02".repeat(32));
const BUILDER_PUB_B64 = "pOCSkrZRwni5dyxWn1-puxPZBrRqtoyd-dwrRAn4ogk";
...
describe("builder pairing (TS)", () => {
  ...
  it("session id + short code vector", () => {
    ...
    expect(sessionId(CODE)).toBe("F2x43pqWEQ9rjC9jLfItSh4RE0K3Izzb");
  ...
  it("derives the pinned public keys", () => {
    expect(base64UrlEncode(x25519.getPublicKey(BUILDER_PRIV))).toBe(BUILDER_PUB_B64);
  ...
  it("derives the pinned SAS + AEAD key (matches Node/Swift/Kotlin)", () => {
    const mat = deriveSessionMaterial(BUILDER_PRIV, base64UrlDecode(PHONE_PUB_B64)!);
    expect(mat.sasCode).toBe(SAS);
    expect(toHex(mat.aeadKey)).toBe(ENC_KEY_HEX);
  });

  it("seal (phone side) → open (builder side) round-trips", () => {
    const builderPub = x25519.getPublicKey(BUILDER_PRIV);
    const phone = deriveSessionMaterial(PHONE_PRIV, builderPub);
    const builder = deriveSessionMaterial(BUILDER_PRIV, x25519.getPublicKey(PHONE_PRIV));
    expect(toHex(phone.aeadKey)).toBe(toHex(builder.aeadKey));
    const plaintext = new TextEncoder().encode('{"hello":"recipe"}');
    const sealed = sealDelivered(plaintext, phone.aeadKey);
    const opened = openDelivered(sealed.ciphertextB64u, sealed.nonceB64u, builder.aeadKey);
    expect(new TextDecoder().decode(opened)).toBe('{"hello":"recipe"}');
  });

  it("qr payload format", () => {
    const p = qrPayload("AEBAGBAF", x25519.getPublicKey(BUILDER_PRIV));
    expect(p).toBe(`flagship://builder?c=AEBAGBAF&k=${BUILDER_PUB_B64}`);
  });
});
