 ...
 *
 * The `live_siblings` name is deliberate: this surface only exposes
 * peers we currently have an authenticated WS to (or that one of those
 * peers gossiped to us right now via sibling-hello.liveSiblings). There
 * is no persisted "siblings I once knew" set — apps must roll with
...
export interface SiblingHttpDeps {
  ...
  /** Long-poll wait window for /api/live_siblings/poll. Default 25s. */
  pollWaitMs?: number;
...
export function buildSiblingHttpHandlers(deps: SiblingHttpDeps) {
  ...
  return async function handle(req: HttpRequest): Promise<HttpResponse | null> {
    ...
    if (req.path === "/api/live_siblings/send" && req.method === "POST") {
