-- voi.ci shortener + user-controlled service URL aliases.
--
-- Two related concepts shipping together:
--
--   1. user_service_aliases — per (username, serviceId) override of the URL
--      stem the service surfaces at. Defaults to the slug-creator derived
--      label; the user can Replace it to hide the package identity
--      and / or rotate after a spam campaign. The internal serviceId
...
--      old codes for the affected service and mints fresh ones, so a
--      previously-shared link breaks the moment the user renames
--      (that's the whole point — adversary's saved URL goes dead).
--
-- A user_service_aliases row is created lazily on first Replace; absent
...
CREATE TABLE IF NOT EXISTS user_service_aliases (
  -- Composite key — one alias per (user, service).
...
-- For "give me every alias for user U" — drives the daemon's services
-- list join + the services-list BFF on /api/users/:u/services/links.
CREATE INDEX IF NOT EXISTS idx_user_service_aliases_username
  ON user_service_aliases(username);
...
CREATE TABLE IF NOT EXISTS voici_links (
  ...
  username TEXT NOT NULL,
  -- Optional serviceId — when present, deleting the service's aliases
  ...
  -- NULL = no expiry (which is what service links use; we manage their
  -- lifecycle via service_id cascade).
  expires_at INTEGER
);

-- For the cascade-delete-on-rename path.
CREATE INDEX IF NOT EXISTS idx_voici_links_user_service
  ON voici_links(username, service_id);
