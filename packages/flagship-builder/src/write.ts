/**
 * `flagship-build write` — raw-disk write of a personalized Flagship ISO
 * onto a USB target.
 *
 * Flow:
...
export async function runWriteCommand(opts: WriteCommandOpts): Promise<WriteCommandResult> {
  ...
  if (!isRoot()) {
    return {
      ok: false,
      reason:
        "raw-disk write requires root. Re-run with: sudo node packages/flagship-builder/src/cli.js write ...",
      exitCode: 13,
    };
  }
