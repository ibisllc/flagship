/**
 * Builder: loadBlob signature + expiry contract.
 *
 * These tests prove the Builder refuses to consume an InstallBlob whose
...
import { BuilderLoadError, loadBlobFromString, debugSshKeyFromGrant } from "../src/loadBlob.js";
...
function buildSignedRecipe(opts: {
  ...
  const blob: InstallBlob = {
    ...
    authCode,
    authCodeUserSignature: new Uint8Array(64), // not verified by the Builder
...
describe("loadBlobFromString", () => {
  ...
  it("accepts (verifies) a blob carrying bootUnlockMode", () => {
    ...
    // or verify fails. Proves the builder round-trip is signature-safe.
  ...
  it("refuses a blob whose authCode is expired", () => {
    ...
    expect(() => loadBlobFromString(json)).toThrow(BuilderLoadError);
    try {
      loadBlobFromString(json);
    } catch (e) {
      expect((e as BuilderLoadError).code).toBe("expired");
    }
  });
  ...
  it("refuses a v1 blob (rejects with malformed/missing-field)", () => {
    ...
    expect(() => loadBlobFromString(v1)).toThrow(BuilderLoadError);
  });
  ...
  it("refuses a blob whose blobSignatureHex doesn't verify", () => {
    ...
    expect(() => loadBlobFromString(tampered)).toThrow(BuilderLoadError);
    try {
      loadBlobFromString(tampered);
    } catch (e) {
      expect((e as BuilderLoadError).code).toBe("bad-signature");
    }
  });

  it("refuses malformed JSON", () => {
    expect(() => loadBlobFromString("{")).toThrow(BuilderLoadError);
    expect(() => loadBlobFromString("[]")).toThrow(BuilderLoadError);
    expect(() => loadBlobFromString("null")).toThrow(BuilderLoadError);
  });
  ...
  it("refuses a blob missing blobSignatureHex", () => {
    ...
    expect(() => loadBlobFromString(noSig)).toThrow(BuilderLoadError);
  });
});
