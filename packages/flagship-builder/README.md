# `@flagship/builder` — flash a Flagship pod onto a USB drive
...
```
phone (trust root) ──signs recipe──┬─ pair session (phone scans the Builder's QR/code) ─┐
                                   ├─ save/share recipe file ─────────────────────────┤→ Builder ──dd──> USB
                                   └─ copy recipe to clipboard ───────────────────────┘   │
                                                                                          └ verifies blob locally; never calls .com
```

The Builder never reaches `flagshipserver.com`. The phone's signature on the
...
at `apps/builder-mac/` and shells out to this CLI.
...
node packages/flagship-builder/src/cli.ts <subcommand> [args]
# (or symlink that into your PATH as `flagship-build`)
```
...
```sh
flagship-build verify ~/Downloads/flagship-recipe-*.json
```
...
```sh
flagship-build verify-iso ~/Downloads/ubuntu-22.04.5-live-server-amd64.iso
```
...
```sh
flagship-build user-data ~/Downloads/recipe.json ./user-data
```
...
```sh
flagship-build prepare ~/recipe.json ~/ubuntu-22.04.iso ~/flagship-ready.iso
...
```sh
sudo flagship-build write ~/recipe.json ~/ubuntu-22.04.iso
# Or non-interactive:
sudo flagship-build write ~/recipe.json ~/ubuntu-22.04.iso --device /dev/disk6 --yes
# Or auto-pick when there's exactly one eligible USB:
sudo flagship-build write ~/recipe.json ~/ubuntu-22.04.iso --device auto --yes
```
...
- **Tampered Builder binary**: covered by reproducible builds + signed releases
