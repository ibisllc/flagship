...
describe("auto-unlock leases — consume serves the lease", () => {
  ...
  it("returns 404 when no lease is present", async () => {
    const irk = makeKey();
    const identity = makeKey();
    const storage = await setup({ irk, identity });
    const res = await consume(storage, identity);
...
// ──────────────────────────────────────────────────────────────────────
// consume — no-lease behavior (relay fallback path)
// ──────────────────────────────────────────────────────────────────────

describe("consume returns 404 when no lease is present", () => {
  it("404s with no lease (boot stage falls back to the relay)", async () => {
    const irk = makeKey();
    const identity = makeKey();
    const storage = await setup({ irk, identity });
    const res = await consume(storage, identity);
  ...
  it("serves the lease and clears a one-shot on consume", async () => {
    const irk = makeKey();
    const identity = makeKey();
    const storage = await setup({ irk, identity });
    await depositLease({ storage, irk, identity, leaseId: "feedface00112233", multiUse: false });
    const r1 = await consume(storage, identity);
    expect(r1.status).toBe(200);
    const r2 = await consume(storage, identity);
    expect(r2.status).toBe(404);
