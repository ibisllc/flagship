...
5. The user's apps are reachable at `<app>.<server>.<user>.flagship.services` over real Let's Encrypt HTTPS — TLS terminates on the user's hardware.
...
- **The daemon** — runs on the user's hardware. Holds the cert. Runs ACME (TLS-ALPN-01) directly with Let's Encrypt over the same SNI passthrough chain that carries production traffic. Serves apps.
...
packages/server-daemon  Production daemon (acme, tunnel client, app runner)
