...
/// Pair-with-the-desktop-Builder screen. Presented (as a sheet) from the
/// create-server delivery chooser once the server is designed. The user
/// scans the QR the Builder shows (or types its short code), confirms the
...
public struct BuilderPairScreen: View {
    @Environment(\.colorScheme) private var scheme
    @Bindable var vm: BuilderPairViewModel
    var onDelivered: (_ serverDomain: String, _ serial: String) -> Void
    var onClose: () -> Void
    var onCancel: () -> Void

    public init(
        vm: BuilderPairViewModel,
    ...
    public var body: some View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: FS.space.s6) {
                ...
                case .connecting:
                    header("Connecting", "Opening the secure channel to the builder…", c: c)
                    spinner(c: c)
                case .matching(let code, let gate):
                    matchPage(code: code, gateOpen: gate, c: c)
                case .delivering:
                    header("Sending", "Minting your recipe and sending it to the builder…", c: c)
                    spinner(c: c)
    ...
    private func scanPage(c: FSColors) -> some View {
        VStack(alignment: .leading, spacing: FS.space.s4) {
            header("Pair with the builder", "On your computer, open the Flagship Studio — it shows a QR code and a short code. Point your camera at it.", c: c)

            FSCard(padding: 0) {
                QRScannerView(
                    onScan: { code in Task { await vm.qrDetected(code) } },
                    onError: { _ in },
                    validate: { BuilderPairing.looksLikeBuilderCode($0) }
                )
                .frame(height: 300)
            ...
            whereToGetBuilder(c: c)
        }
    }

    private func enterCodePage(c: FSColors) -> some View {
        VStack(alignment: .leading, spacing: FS.space.s4) {
            header("Enter the builder code", "Type the short code shown under the QR on your computer (like ABCD-EFGH).", c: c)
            FSCard {
                FSField(value: $vm.typedCode, label: "Builder code", placeholder: "ABCD-EFGH")
    ...
    private func whereToGetBuilder(c: FSColors) -> some View {
        FSCard {
        ...
        VStack(alignment: .leading, spacing: FS.space.s4) {
            ...
            FSCard {
                VStack(spacing: FS.space.s3) {
                    Text(BuilderPairing_format(code))
...
/// builder-mac copy. Mirrors BuilderPairing.formatMatchCode.
private func BuilderPairing_format(_ code: String) -> String {
