...
public final class AppsListViewModel {
    ...
    /// set to a specific podId, the apps list is filtered to apps
    /// running on that pod. Today's daemon AppsListResponse doesn't
    /// yet carry a `installedOn` field per app, so we approximate by
    ...
    /// V7 — pods the user has on file. The Apps tab passes the
    ...
    public var filteredApps: [FlagshipAPI.AppSummary] {
        ...
        if let podId = serverFilter,
           let podName = availablePods.first(where: { $0.podId == podId })?.name {
            ...
            // Match apps whose canonical URL contains the pod name as
