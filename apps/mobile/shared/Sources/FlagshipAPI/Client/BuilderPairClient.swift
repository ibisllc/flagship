...
/// bidirectional deposit session with the desktop builder (relay DO:
/// apps/com/src/builderRelay.ts). The phone learns the builder's pubkey,
/// confirms the SAS, and delivers the recipe. A brief phone-side socket loss
/// reconnects to the same relay slot while pairing; once the builder returns a
/// successful staging receipt, the phone has no further role.

/// Decoded inbound events the phone cares about.
public enum BuilderInbound: Sendable, Equatable {
    case accepted
    case peerPresent           // the builder was already connected when we joined
    case peerJoined            // the builder joined after us
    case builderHello(builderPkB64: String)
    case recipeAccepted        // the builder successfully staged the recipe
...
/// Frames the phone sends to the builder (forwarded verbatim by the relay).
public enum BuilderOutbound: Sendable {
...
public protocol BuilderPairClient: Sendable {
    /// Open the WS as role=phone and return a stream of inbound events.
    func connect(sid: String) async throws -> AsyncStream<BuilderInbound>
    func send(_ frame: BuilderOutbound) async throws
    func close() async
}

public enum BuilderPairError: Error, LocalizedError, Sendable {
...
public final class LiveBuilderPairClient: BuilderPairClient, @unchecked Sendable {
    ...
    private var continuation: AsyncStream<BuilderInbound>.Continuation?
    ...
    public func connect(sid: String) async throws -> AsyncStream<BuilderInbound> {
        let encodedSid = sid.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? sid
        guard let url = URL(string: "\(scheme)://\(host)/builder-pipe/\(encodedSid)?role=phone") else {
            throw BuilderPairError.connectionFailed("bad URL")
        }
        let t = urlSession.webSocketTask(with: url)
        var streamContinuation: AsyncStream<BuilderInbound>.Continuation!
        let stream = AsyncStream<BuilderInbound> { cont in
    ...
    public func send(_ frame: BuilderOutbound) async throws {
        guard let t = currentTask() else {
            throw BuilderPairError.connectionFailed("the connection is closed")
        }
        do {
            try await t.send(.string(frame.json))
        } catch {
            throw BuilderPairError.connectionFailed(Self.describe(error))
        }
    }
    ...
    private typealias Connection = (
        ...
        continuation: AsyncStream<BuilderInbound>.Continuation?
    )

    private func replaceConnection(
        task newTask: URLSessionWebSocketTask,
        continuation newContinuation: AsyncStream<BuilderInbound>.Continuation,
        id: UUID
    ) -> Connection {
        stateLock.lock()
    ...
    private func yield(_ ev: BuilderInbound, id: UUID) {
        stateLock.lock()
    ...
    private func finishConnection(id: UUID, event: BuilderInbound) {
        stateLock.lock()
    ...
    private static func decode(_ message: URLSessionWebSocketTask.Message) -> BuilderInbound? {
        let data: Data
        switch message {
        ...
        case "peer":
            ...
            switch fkind {
            case "builder-hello":
                guard let pk = frame["builderPk"] as? String else { return nil }
                return .builderHello(builderPkB64: pk)
...
public final class MockBuilderPairClient: BuilderPairClient, @unchecked Sendable {
    public private(set) var connectedSid: String?
    public private(set) var connectCount = 0
    public private(set) var sent: [BuilderOutbound] = []
    ...
    public func send(_ frame: BuilderOutbound) async throws { sent.append(frame) }
    ...
    public func emit(_ ev: BuilderInbound) { continuation?.yield(ev) }
