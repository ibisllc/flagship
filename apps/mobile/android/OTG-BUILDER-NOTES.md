# On-device USB-OTG builder (Android) — design notes & findings
...
## 1. What the desktop builder does (the thing we are mimicking)

Reference: `apps/builder-mac/Sources/FlagshipBuilderCore/{IsoBaseCache,IsoManifestClient,Remaster,UserData}.swift`
and `packages/flagship-builder/src/{write,preseed,userdata,remasterIso}.ts`.

The desktop flow is:

1. **Manifest** — `POST https://flagshipserver.com/api/iso-manifest` with
   `{platform, builderVersion, current:{version,sha256}|null}`; the server replies
   `{download:{url,sha256,version,sizeBytes,attestation}}` or `{download:null}`
   ("keep what you have"). The builder is a dumb executor — it never compares shas
...
2. **Download + verify** — stream the ordered URL to a cache file, computing
   ...
   `~/Library/Caches/flagship-builder/flagship-base-<version>.iso`.
...
All under `app/src/main/java/com/flagshipserver/app/builder/`:
...
- **`iso/IsoManifest.kt` + `IsoManifestClient.kt`** — the locked wire contract,
  over an injectable `BuilderHttp` seam. (`IsoManifestClientTest` — fake HTTP.)
...
- **`BuilderOnDeviceViewModel.kt`** — orchestrates detect → permission → download
  → verify → inject → write, exposing a `StateFlow<BurnState>`.
- **`ui/screens/BuilderOnDeviceScreen.kt`** — the Compose UI: drive picker,
  "this erases the drive" warning, phase progress bar, success — public entry
  `BuilderOnDeviceScreen(recipeJson, onDone)`.
...
2. **The preseed-text source for Android.** The signed preseed/bootstrap text
   MUST come from the **shared generator** (`packages/flagship-builder`
   ...
   phone's signature stays the trust root, matching the desktop builder).
