# Flagship Burner — macOS GUI

Native SwiftUI wrapper around the `@flagship/burner` Node CLI
(`packages/flagship-burner/`). For users who'd rather click than type.
...
`packages/flagship-burner/src/`. The app finds it relative to its
...
cd apps/burner-mac
swift build
swift run FlagshipBurner
```

To override the CLI entry point during development:

```sh
FLAGSHIP_BURN_ENTRY=/path/to/cli.ts swift run FlagshipBurner
```
...
Package.swift
Sources/
  FlagshipBurnerCore/   pure, testable
    CLIArgs              flagship-burn arg-vector builders
  ...
  FlagshipBurner/       SwiftUI + AppKit shell
