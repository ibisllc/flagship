"""Phone pairing: drive `flagship-burn pair --emit-events` and surface its
...
(packages/flagship-burner/src/pair.ts) — this is thin glue, exactly like the
remaster/write path. Mirrors apps/burner-windows/src/PairSession.cs.
