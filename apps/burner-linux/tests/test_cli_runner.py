...
def test_find_entry_skips_missing_env_override_then_falls_to_dev_path(tmp_path, monkeypatch):
    # Build the apps/burner-linux/../../packages/flagship-burner/src/cli.ts path
    # under a fake repo root.
    fake_repo = tmp_path / "flagship-fake"
    burner_dir = fake_repo / "apps" / "burner-linux"
    burner_dir.mkdir(parents=True)
    cli_src = fake_repo / "packages" / "flagship-burner" / "src" / "cli.ts"
    cli_src.parent.mkdir(parents=True)
    cli_src.write_text("// stub\n")

    e = find_entry(
        executable_dir=burner_dir,
