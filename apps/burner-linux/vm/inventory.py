...
Mirrors apps/burner-windows/src/VM/VMInventoryStore.cs with Linux-native
...
class VMBundleLayout:
    ...
    def default_root() -> "VMBundleLayout":
        """Production default: $XDG_DATA_HOME/flagship-burner/VMs
        (falls back to ~/.local/share/…)."""
        data_home = os.environ.get("XDG_DATA_HOME")
        if not data_home:
            data_home = str(Path.home() / ".local" / "share")
        return VMBundleLayout(str(Path(data_home) / "flagship-burner" / "VMs"))
