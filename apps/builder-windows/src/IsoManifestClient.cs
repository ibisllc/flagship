...
/// builders' manifest client (apps/builder-mac IsoManifestClient.swift,
/// apps/builder-linux iso_manifest_client.py). Same locked wire contract:
///
///   POST https://flagshipserver.com/api/iso-manifest
///   Request  { platform, builderVersion, current: {version, sha256} | null }
///   Response { download: {url, sha256, version, sizeBytes, attestation} | null }
///
/// The builder is a DUMB EXECUTOR: it reports the SHA256 of whatever base ISO it
...
public sealed class IsoManifestClient
{
    ...
    /// <summary>The platform tag this builder reports.</summary>
    ...
    /// <summary>The current cached base ISO the builder is reporting, or null.</summary>
    ...
    /// <summary>
    /// POST the manifest with the builder version + the currently-cached base (or
    ...
    {
        string body = SerializeRequest(builderVersion, current);
    ...
    /// </summary>
    public static string SerializeRequest(string builderVersion, Current? current)
    {
        ...
        using (var w = new Utf8JsonWriter(ms))
        {
            ...
            w.WriteString("builderVersion", builderVersion);
            if (current == null)
            {
                w.WriteNull("current");
            }
