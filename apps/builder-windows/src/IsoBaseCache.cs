...
/// <summary>
/// Manifest-driven base-ISO cache. Pure-C# port of the sibling builders'
/// manifest-driven cache (apps/builder-mac IsoBaseCache.swift,
/// apps/builder-linux iso_base_cache.py). Replaces the retired hardcoded-Alpine
/// BaseIsoCache (recovered for mechanics only via
/// `git show alpine:apps/builder-windows/src/BaseIsoCache.cs`).
///
/// The builder is a DUMB EXECUTOR. On every Simple-mode bake it:
...
public sealed class IsoBaseCache
{
    ...
    private readonly string _builderVersion;
    ...
    /// <param name="builderVersion">Reported in the manifest request.</param>
        ...
        string endpoint = IsoManifestClient.DefaultEndpoint)
    {
        ...
        _builderVersion = builderVersion;
        Directory.CreateDirectory(_cacheDir);
    }

    /// <summary>%LOCALAPPDATA%\flagship-builder (created on demand).</summary>
    public static string DefaultCacheDir()
    {
        ...
        return Path.Combine(local, "flagship-builder");
    }
        ...
        CancellationToken cancellation = default)
    {
        ...
        try
        {
            manifest = await _client.FetchAsync(_builderVersion, current, cancellation).ConfigureAwait(false);
        }
