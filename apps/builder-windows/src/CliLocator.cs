...
/// <summary>
/// Find Node + the Flagship Studio CLI entry point on Windows.
///
...
///      ..\..\..\..\packages\flagship-builder\src\cli.ts (dev) or
///      a bundled dist\cli.js (release packaging — Phase 2)
///
/// Mirrors apps/builder-mac CLILocator.swift + apps/builder-linux cli_runner.py.
/// </summary>
...
public static class CliLocator
{
                                    ...
                                    string? executableDir = null)
    {
        ...
        // dev run: apps\builder-windows\bin\Debug\net8.0-windows\ → walk
        // up to the repo root then into packages\flagship-builder\src\cli.ts.
        ...
        for (var depth = 1; depth <= 6; depth++)
        {
            ...
            var candidate = Path.Combine(ancestor,
                "packages", "flagship-builder", "src", "cli.ts");
            searched.Add(candidate);
            if (exists(candidate)) return candidate;
            var jsCandidate = Path.Combine(ancestor,
                "packages", "flagship-builder", "dist", "cli.js");
        ...
        var local = Path.Combine(baseDir, "flagship-builder", "src", "cli.ts");
        searched.Add(local);
        if (exists(local)) return local;
        var localJs = Path.Combine(baseDir, "flagship-builder", "dist", "cli.js");
        searched.Add(localJs);
        if (exists(localJs)) return localJs;
        throw new CliLocatorException(
            $"Flagship Studio CLI entry not found; searched: {string.Join("; ", searched)}");
    }
