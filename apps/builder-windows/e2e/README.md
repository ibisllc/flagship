...
npx tsx apps/builder-windows/e2e/mint-recipe.ts out\recipe.json

# 2. Fetch + remaster the base ISO (the same CLI path the app's Host-here uses).
#    The manifest is live, so the base downloads to %LOCALAPPDATA%\flagship-builder\.
$env:Path = "C:\msys64\usr\bin;$env:Path"
npx tsx packages/flagship-builder/src/cli.ts prepare out\recipe.json `
    "$env:LOCALAPPDATA\flagship-builder\flagship-base-debian-13.5.0.iso" out\installer.iso --keep-recipe

# 3. Boot it end-to-end.
dotnet run --project apps/builder-windows/e2e/VmE2E -- out\recipe.json out\installer.iso out\vms
```
...
1. **Bake a diagnostic dev SSH key** (now wired end-to-end). The CLI's
   ...
   npx tsx packages/flagship-builder/src/cli.ts prepare out\recipe.json `
       "$env:LOCALAPPDATA\flagship-builder\flagship-base-debian-13.5.0.iso" `
