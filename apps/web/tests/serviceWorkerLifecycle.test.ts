...
describe("service-worker.js — install / activate / waiting-state safety", () => {
  ...
  it("activate deletes stale shell caches but keeps the current one", async () => {
    ...
    await h.caches.open("flagship-webapp-shell-v10");
    await h.caches.open("flagship-webapp-shell-v11");
    await fireEvent(h.self, "install", {}); // creates -v12
    await fireEvent(h.self, "activate", {});
    const remaining = await h.caches.keys();
    expect(remaining).toContain("flagship-webapp-shell-v12");
    expect(remaining).not.toContain("flagship-webapp-shell-v10");
    expect(remaining).not.toContain("flagship-webapp-shell-v11");
  });
