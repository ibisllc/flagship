...
test("S2 — pair the webapp with the pod-sim", async ({ page, identity, podSim }) => {
  ...
  const raw = order.raw as { serverId: string; token: string };
  ...
  // 5. The webapp should have persisted the pair in the active profile.
  const persisted = await page.evaluate(async () => {
    const api = (await import("/lib/api.js" as string)) as {
      getPodBaseUrl(): string;
      getSessionToken(): string;
    };
    return { podBaseUrl: api.getPodBaseUrl(), sessionToken: api.getSessionToken() };
  });
  expect(persisted.podBaseUrl).toBe(podSim.baseUrl);
  expect(persisted.sessionToken).toBe(raw.token);
});
