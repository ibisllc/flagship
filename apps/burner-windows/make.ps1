#!/usr/bin/env pwsh
# Build, test, and publish the Flagship Burner Windows GUI.
#
# Usage:
...
function Invoke-Build {
    ...
    & dotnet build "$ScriptDir/FlagshipBurner.csproj"
    if ($LASTEXITCODE -ne 0) { throw "build failed" }
}

function Invoke-Test {
    Write-Host "==> dotnet test" -ForegroundColor Cyan
    & dotnet test "$ScriptDir/tests/FlagshipBurner.Tests.csproj" --logger "console;verbosity=normal"
    if ($LASTEXITCODE -ne 0) { throw "tests failed" }
}

function Invoke-Publish {
    Write-Host "==> dotnet publish (Release, win-x64, self-contained)" -ForegroundColor Cyan
    & dotnet publish "$ScriptDir/FlagshipBurner.csproj" `
        -c Release `
        -r win-x64 `
        --self-contained true `
    ...
    Write-Host "==> built: $ScriptDir/dist/FlagshipBurner.exe" -ForegroundColor Green
}
...
function Invoke-Sign {
    ...
    $exe = "$ScriptDir/dist/FlagshipBurner.exe"
