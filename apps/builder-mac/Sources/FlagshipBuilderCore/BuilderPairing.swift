...
/// Phone↔builder pairing — the builder (initiator) side.
///
/// The builder shows a QR + a short human code. The phone scans/types it,
...
/// builder forwards its ephemeral public key over the relay (`builder-hello`)
...
/// only builder-specific addition is the short-code → session-id mapping
/// (`flagship/builder-sid/v1`), pinned by a cross-platform vector.
///
...
/// The live socket is the gate — when it drops, the builder re-locks.
public enum BuilderPairing {
    ...
    public enum PairingError: Error, LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .badShortCode: return "That code isn't valid — check it on the builder."
    ...
    /// `<sid>` in `/builder-pipe/<sid>`. base64url of SHA256(tag || code),
    ...
    public static func sessionId(forCodeBytes code: Data) -> String {
        ...
        return String(Base64URLBuilder.encode(digest).prefix(32))
    }
    ...
    /// The QR payload the builder displays. Carries the short code AND the
    /// builder's ephemeral X25519 pubkey so a scanning phone needs no relay
    /// round-trip to derive the SAS.
    public static func qrPayload(humanCode: String, builderPublicKey: Data) -> String {
        "flagship://builder?c=\(humanCode)&k=\(Base64URLBuilder.encode(builderPublicKey))"
    }
    ...
    /// Builder-side X25519 + HKDF. `builderPrivateKey` is this session's
    ...
    public static func deriveMaterial(
        builderPrivateKey: Curve25519.KeyAgreement.PrivateKey,
        phonePublicKey: Data
        ...
        do { shared = try builderPrivateKey.sharedSecretFromKeyAgreement(with: peer) }
    ...
    public static func open(
        ...
        guard let ctData = Base64URLBuilder.decode(ct), let nonceData = Base64URLBuilder.decode(nonce),
...
/// base64url (RFC 4648 §5, no padding) — local copy so FlagshipBuilderCore
/// has no cross-package dependency.
public enum Base64URLBuilder {
