# Flagship Studio — Linux GUI

GTK4 + libadwaita wrapper around the `@flagship/builder` Node CLI
(`packages/flagship-builder/`). UX matches `apps/builder-mac/` 1:1: one
...
(parity with `apps/builder-windows` and `apps/builder-mac`).

## Two modes

Both modes remaster a base ISO with your signed recipe via the same
`@flagship/builder` Node CLI `write` subcommand, then flash it to a USB drive.
They differ only in *where the base ISO comes from*:

- **Simple (default)** — you don't supply an ISO. The builder fetches a stock
  **Debian-netinst** base ISO chosen by the **server manifest**
  (`POST https://flagshipserver.com/api/iso-manifest`), verifies its sha256,
  and caches it under `~/.cache/flagship-builder/`. Every later burn reuses the
  cache; the server decides when a new base ships. The builder is a *dumb
...
1. **Recipe** — drag in (or paste) the signed JSON the website produces
   ...
   `flagship-build verify` and shows you the server-domain + expiry so
   you can sanity-check before flashing.
2. **ISO** — *Advanced only.* Drag in a stock Ubuntu/Debian Server ISO. Run
   `flagship-build distros` for accepted SHAs. In **Simple** mode this step is
...
- **Recipes can also arrive by phone pairing** — "Pair with your phone" drives
  the shared `flagship-build pair --emit-events` CLI and renders the QR / code /
...
- **Debug is consent-as-crypto.** SSH + the serial console exist IFF the recipe
  ...
  toggle, or `flagship-build pair --debug`). The grant's `sshAuthorizedKey` is
...
- Bundles live under `$XDG_DATA_HOME/flagship-builder/VMs/<fqdn>/`
...
Extra requirements for hosting (on top of the builder's):
...
- **`pkexec`** (PolicyKit) — ships on every modern desktop distro (NOT in
  ChromeOS's stock container — there the builder falls back to passwordless
  `sudo -n`, which Crostini grants the primary user; see below).

### ChromeOS (Crostini)

The builder runs inside ChromeOS's Linux container, and **Host on this PC gets
...
```sh
# From the repo root:
python3 apps/builder-linux/flagship-builder.py
```

The GUI resolves the Node CLI entry from
`packages/flagship-builder/dist/cli.js` relative to its own directory —
run `npx tsc -b packages/flagship-builder` once so the build exists
...
```sh
python3 -m pytest apps/builder-linux/tests/
```
...
with `flagship-builder.py` (GTK objects are only constructed inside
`build_window`, so everything above runs headless without `gi`).

Static analysis:

```sh
python3 -m py_compile apps/builder-linux/*.py
```
...
# → apps/builder-linux/dist/FlagshipBuilder-x86_64.AppImage
```

The AppImage bundles the Python sources + the built CLI (`tsc -b`
output from `packages/flagship-builder/dist/`). It does **not** bundle
...
```sh
sudo install -m 644 \
    apps/builder-linux/polkit/com.flagshipserver.builder.policy \
    apps/builder-linux/polkit/com.flagshipserver.builder.write-image.policy \
...
flatpak-builder --user --install build-dir \
    apps/builder-linux/flatpak/com.flagshipserver.Builder.yaml
```
...
```
flagship-builder.py    GTK app entry — wires Adw.Application, opens the wizard window
...
pair_session.py       FLAGSHIP_PAIR NDJSON event parser + pair-session driver
                      (wraps `flagship-build pair --emit-events`)
vm/                   the VM appliance host layer (mirrors builder-windows src/VM/):
