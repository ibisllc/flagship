...
The builder keeps at most one stock Debian-netinst base ISO under
$XDG_CACHE_HOME/flagship-builder (fallback ~/.cache/flagship-builder). On every
Simple bake it:
...
The builder is a DUMB EXECUTOR: it NEVER decides-by-sha client-side. It reports
...
def cache_dir() -> Path:
    """$XDG_CACHE_HOME/flagship-builder, falling back to ~/.cache/flagship-builder
    per the XDG Base Directory spec."""
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg) if xdg else (Path.home() / ".cache")
    return base / "flagship-builder"
...
def _download(
    ...
    req = Request(descriptor.url, headers={"User-Agent": "flagship-builder-linux"})
    try:
        resp = do_open(req)
...
def ensure(
    ...
    *,
    ...
    try:
        result = fetch(builder_version, current, arch=arch)
