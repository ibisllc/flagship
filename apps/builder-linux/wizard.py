"""GTK4 + libadwaita wizard module — UX-matches apps/builder-mac 1:1.
...
and a log pane pinned at the bottom:
  ...
  4. Big "Bake" button → calls `flagship-build write` and streams output
...
def locate_flasher() -> str:
    ...
    AppImage's flagship-builder share dir). Override with $FLAGSHIP_FLASHER."""
    ...
    for c in (
        Path("/usr/share/flagship-builder/disk_write.py"),
        here / "disk_write.py",
    ):
        if c.exists():
            return str(c)
...
class WizardModel:
    ...
    def start_pairing(self) -> None:
        """Spawn `flagship-build pair --emit-events`, surface the QR / code /
...
def build_window(application, model: Optional[WizardModel] = None):
    ...
    window.set_title("Flagship Studio")
    ...
    step2 = _wizard_card(
        step=2,
        title="Drop the ISO",
        subtitle="Ubuntu Server stock image. Run `flagship-build distros` for accepted SHAs.",
    )
