"""Spawn the @flagship/builder Node CLI, stream stdout/stderr.

Mirrors apps/builder-mac/Sources/FlagshipBuilderCore/CLIRunner.swift +
...
def find_entry(
    ...
    """Find the flagship-build CLI entry. Searches:
      1. $FLAGSHIP_BURN_ENTRY
      2. ../../packages/flagship-builder/dist/cli.js from this file (the tsc
         build — plain `node` runs it; node 20 can NOT execute the .ts)
      3. ../../packages/flagship-builder/src/cli.ts (last-resort: only works
         under a TS-capable runtime)
      4. /usr/share/flagship-builder/cli.js (system-installed AppImage extract)
      5. /usr/share/flagship-builder/cli.ts
    """
    env = environ if environ is not None else os.environ
    ...
    # apps/builder-linux/ → walk up to flagship/, then over to
    # packages/flagship-builder/. dist BEFORE src: the runner is plain `node`,
    # which throws ERR_UNKNOWN_FILE_EXTENSION on the .ts — a checkout that ran
    # `npx tsc -b` must Just Work.
    candidates = [
        base.parent.parent / "packages" / "flagship-builder" / "dist" / "cli.js",
        base.parent.parent / "packages" / "flagship-builder" / "src" / "cli.ts",
        Path("/usr/share/flagship-builder/cli.js"),
        Path("/usr/share/flagship-builder/cli.ts"),
        Path("/usr/share/flagship-builder/dist/cli.js"),
        Path("/usr/share/flagship-builder/src/cli.ts"),
    ]
    for c in candidates:
        if fileexists(str(c)):
            return str(c)
    raise CLILocateError(
        f"flagship-build entry not found; searched: {[str(c) for c in candidates]}"
    )
