...
The builder is a DUMB EXECUTOR. It POSTs its current cached-base state to the
...
The builder NEVER decides by sha client-side; it only *reports* `current` and
...
class CurrentBase:
    """What the builder currently has cached. Reported as `current`."""
    version: str
    sha256: str
...
class ManifestResult:
    ...
    tells the builder to keep its cache."""
    ...
    *,
    ...
    req = Request(
        ...
        headers={
            ...
            "User-Agent": "flagship-builder-linux",
        },
    )
