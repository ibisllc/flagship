...
/**
 * Canonical cross-platform vector for the phone↔burner pairing crypto.
 *
 * The burner (Swift, apps/burner-mac) and the phone (Swift/Kotlin) each
 ...
 * (`flagship/burner-sid/v1`). This test recomputes the vector from fixed
 * raw keys with Node's `crypto` and pins the outputs; the Swift burner test
 * (BurnerPairingTests.test_crossPlatformVector) asserts the IDENTICAL
 * constants. If either side moves a salt/info/tag, one of the two breaks.
 *
 * The relay DO itself (burnerRelay.ts) is crypto-blind — it never sees keys
...
describe("burner pairing — cross-platform crypto vector", () => {
  const burnerRaw = Buffer.from("01".repeat(32), "hex");
  const phoneRaw = Buffer.from("02".repeat(32), "hex");
  const burnerPriv = privFromRaw(burnerRaw);
  const phonePriv = privFromRaw(phoneRaw);

  it("derives the pinned public keys, SAS and AEAD key", () => {
    expect(b64url(rawPub(burnerPriv))).toBe("pOCSkrZRwni5dyxWn1-puxPZBrRqtoyd-dwrRAn4ogk");
    expect(b64url(rawPub(phonePriv))).toBe("zo060cy2M-x7cMF4FKXHbs0CloUFDTRHRboFhw5YfVk");

    const shared = crypto.diffieHellman({ privateKey: burnerPriv, publicKey: crypto.createPublicKey(phonePriv) });
  ...
  it("derives the pinned session id + short code from the code bytes", () => {
    ...
    const sidFull = crypto.createHash("sha256")
      .update(Buffer.concat([Buffer.from("flagship/burner-sid/v1"), codeBytes]))
      .digest();
    expect(b64url(sidFull).slice(0, 32)).toBe("KW3_KaK0uN8rcrQCLmsOJXXfhr9EEpib");
