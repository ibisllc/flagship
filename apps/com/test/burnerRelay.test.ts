...
import { BurnerRelaySession, _internal } from "../src/burnerRelay.js";

/**
 * Burner relay — tests for the phone↔desktop-burner live-session DO.
...
function makeState(id = "burner-session-aaaaaaaaaaaaaaaa") {
  ...
  return {
    ...
    _bindDo(d: BurnerRelaySession): void { doRef = d; },
    _attached: attached,
  };
}
type TestState = ReturnType<typeof makeState>;

function makeBound(id?: string): { state: TestState; obj: BurnerRelaySession } {
  const state = makeState(id);
  const obj = new BurnerRelaySession(state as any, {});
  state._bindDo(obj);
  return { state, obj };
}

async function upgrade(
  obj: BurnerRelaySession,
  role: "burner" | "phone",
): Promise<FakeSocket> {
...
describe("burner relay — join + presence", () => {
  it("accepts a burner upgrade on a free session", async () => {
    const { obj } = makeBound();
  ...
  it("tells both sides about presence when the phone joins after the burner", async () => {
    ...
    // Newcomer learns the burner is already present.
    const present = await phone.waitFor((m) => m.kind === "peer-present");
    expect(present.kind).toBe("peer-present");
    // The existing burner is told the phone joined.
    const joined = await burner.waitFor((m) => m.kind === "peer-joined");
...
describe("burner relay — bidirectional forwarding", () => {
  it("forwards a burner app frame to the phone wrapped as {kind:peer,frame}", async () => {
    const { obj } = makeBound();
    const burner = await upgrade(obj, "burner");
    await burner.waitFor((m) => m.kind === "accepted");
    const phone = await upgrade(obj, "phone");
    ...
    burner.push(JSON.stringify({ kind: "burner-hello", burnerPk: "QUJD" }));
    const fwd = await phone.waitFor((m) => m.kind === "peer");
    expect(fwd.frame).toEqual({ kind: "burner-hello", burnerPk: "QUJD" });
  });

  it("forwards a phone app frame to the burner (the reverse direction)", async () => {
    const { obj } = makeBound();
    const burner = await upgrade(obj, "burner");
    await burner.waitFor((m) => m.kind === "accepted");
    const phone = await upgrade(obj, "phone");
    ...
    const fwd = await burner.waitFor((m) => m.kind === "peer");
  ...
  it("tells a sender peer-missing when no peer is connected", async () => {
    ...
    await burner.waitFor((m) => m.kind === "accepted");
    burner.push(JSON.stringify({ kind: "burner-hello", burnerPk: "QUJD" }));
    const m = await burner.waitFor((x) => x.kind === "peer-missing");
  ...
  it("does not forward relay control frames as peer frames (ping is handled, not relayed)", async () => {
    ...
    burner.push(JSON.stringify({ kind: "ping" }));
    const pong = await burner.waitFor((m) => m.kind === "pong");
...
describe("burner relay — keepalive", () => {
...
describe("burner relay — presence loss → re-lock", () => {
  it("tells the survivor peer-gone when the other side closes", async () => {
    ...
    const gone = await burner.waitFor((m) => m.kind === "peer-gone");
  ...
  it("eagerly wipes storage when both peers have left", async () => {
    ...
    phone.close(1000, "x");
    await burner.waitFor((m) => m.kind === "peer-gone");
    expect(state.storage._state.kv.size).toBeGreaterThan(0); // burner still here
    burner.close(1000, "x");
    await new Promise((r) => setTimeout(r, 10));
...
describe("burner relay — validation", () => {
  it("errors on a malformed (non-JSON) frame", async () => {
    const { obj } = makeBound();
    const burner = await upgrade(obj, "burner");
    await burner.waitFor((m) => m.kind === "accepted");
    burner.push("not json");
    const err = await burner.waitFor((m) => m.kind === "error");
  ...
  it("errors on a frame over the size cap", async () => {
    ...
    burner.push(JSON.stringify({ kind: "deliver", blob: huge }));
    const err = await burner.waitFor((m) => m.kind === "error");
...
describe("burner relay — TTL + hibernation", () => {
  ...
  it("persists createdAt so a reconstructed (hibernated) DO honours the same lifetime", async () => {
    ...
    const obj2 = new BurnerRelaySession(state as any, {});
    state._bindDo(obj2);
  ...
  it("refuses upgrades after the absolute TTL", async () => {
    ...
    const r = await obj.fetch(new Request("https://do.internal/x?role=burner", {
      method: "GET",
      headers: { upgrade: "websocket" },
    }));
  ...
  it("alarm() notifies open sockets with {expired} and wipes storage", async () => {
    ...
    await burner.waitFor((m) => m.kind === "accepted");
    await obj.alarm();
    const expired = await burner.waitFor((m) => m.kind === "expired");
  ...
  it("accepts sockets via state.acceptWebSocket with a role tag (hibernation API)", async () => {
    ...
    expect(state._attached[0]!.tags).toContain("burner");
    expect((state._attached[0]!.ws as FakeSocket).deserializeAttachment()).toEqual({ role: "burner" });
  });

  it("source: uses the hibernation API, not legacy ws.accept()", async () => {
    const fs = await import("fs/promises");
    const url = new URL("../src/burnerRelay.ts", import.meta.url);
